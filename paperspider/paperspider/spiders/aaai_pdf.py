...
class AaaiPdfSpider(scrapy.Spider):
    name = "aaaipdf"
    start_urls = []
    headers = {
    ...
    def get_urls_from_db(self, db):
        ...
        cursor = conn.cursor()
        cursor.execute(
            'SELECT pdf_url,author,year,journal,id FROM crawler_info WHERE journal=? and file_url=?', ("aaai",""))
        self.aaai = cursor.fetchall()
        cursor.close()
        conn.close()

    def start_requests(self):
        for row in self.aaai:
            author = row[1]
            for _ in "/\:*\"<>|?":
